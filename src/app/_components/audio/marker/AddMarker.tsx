...
interface AddMarkerProps {
  currentTime: number;
  onAddMarker: (label: string, startTime: number, endTime?: number | null) => void;
  selectedRegion?: { start: number | null; end: number | null };
  onClearRegion?: (() => void) | null;
}

export default function AddMarker({ currentTime, onAddMarker, selectedRegion, onClearRegion }: AddMarkerProps) {
  const [newMarkerLabel, setNewMarkerLabel] = useState('');
  const [isSection, setIsSection] = useState(false);
  const roundedStartTime = selectedRegion?.start != null ? roundTime(selectedRegion.start) : roundTime(currentTime);
  const [sectionEndTime, setSectionEndTime] = useState<number>(roundedStartTime + 10);
  const t = useTranslations('AddMarker');
  
  // Auto-enable section switch when a region is selected
  useEffect(() => {
    if (selectedRegion?.start != null && selectedRegion?.end != null && selectedRegion.end > selectedRegion.start) {
      setIsSection(true);
    }
  }, [selectedRegion]);
  
  // Update section times when region is selected
  const effectiveStartTime = selectedRegion?.start != null ? roundTime(selectedRegion.start) : roundedStartTime;
  const effectiveEndTime = selectedRegion?.end != null && selectedRegion.end > effectiveStartTime 
    ? roundTime(selectedRegion.end) 
    : sectionEndTime;

  const handleAdd = () => {
    onAddMarker(newMarkerLabel, effectiveStartTime, isSection ? effectiveEndTime : null);
    setNewMarkerLabel('');
  };

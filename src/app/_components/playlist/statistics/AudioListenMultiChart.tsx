          ...
          {payload
            ...
            .map((entry) => (
              <p key={entry.dataKey} className="text-xs" style={{ color: entry.color }}>
                {entry.name}: {t('chart.tooltipListens', { count: entry.value })}
              </p>
            ))}
...
export function AudioListenMultiChart({ audioStats }: AudioListenMultiChartProps) {
  ...
  const chartData = Array.from(dateMap.entries())
    ...
    .map(([date, values]) => ({
      date,
      ...Object.fromEntries(audioStats.map(a => [a.audioId, values[a.audioId] ?? 0])),
    }));
